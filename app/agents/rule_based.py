...
from app.models.domain import Evidence, FactExtractionResult, FactStatus, LeadFact
...
_SIZE_PATTERNS = (
    re.compile(
        r"(?:team of|over|about|approximately|more than)?\s*([\d,]+)\+?\s+employees",
        re.I,
    ),
...
class RuleBasedFactExtractor:
    ...
    def _designation_fact(self, lead: LeadInput, evidence: list[Evidence]) -> LeadFact:
        ...
        supporting = [
            ...
            and (
                lead.name.casefold() in item.excerpt.casefold()
                or lead.company.casefold() in item.excerpt.casefold()
            )
        ]
        ...
        if supporting:
            return LeadFact(
                ...
                evidence_ids=supporting,
                rationale=(
                    "The submitted title appears with the person or company in public evidence."
                ),
            )
        return LeadFact(
            field="designation",
    ...
    def _reputation_fact(self, evidence: list[Evidence]) -> LeadFact:
        ...
        return LeadFact(
            ...
            rationale=(
                "Reputation is derived from source diversity and authoritative public presence."
            ),
        )

    def _business_relevance_fact(
        self,
        lead: LeadInput,
        evidence: list[Evidence],
    ) -> LeadFact:
        ...
        if evidence:
            return LeadFact(
                ...
                rationale=(
                    "Public company information exists, but no target keywords were configured."
                ),
            )
